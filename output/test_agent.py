...
These tests check the core logic (parsing, scoring, ranking) WITHOUT
calling the Gemini API -- so they run instantly, for free, and don't
need an API key. This is intentional: automated tests should be fast
and deterministic, unlike LLM calls which cost money and can vary.
"""
...
def test_sample_data_exists():
    """Sanity check: the sample JD and at least 10 resumes are present."""
...
def test_extract_text_reads_txt_file():
    """extract_text() should return non-empty text for a .txt resume."""
...
def test_extract_skill_keywords_finds_known_skills():
    """The JD mentions Python and AWS -- both should be detected."""
...
def test_similarity_score_is_higher_for_strong_match():
    """
    Core correctness check: a resume full of matching keywords (Priya,
    a strong Python/Django/AWS backend engineer) must score higher than
    a resume from a completely unrelated field (Sneha, HR).
    """
    jd_text = extract_text(JD_PATH)
...
def test_skill_match_breakdown_matched_and_missing_dont_overlap():
    """A skill can't be both matched and missing at the same time."""
    jd_text = extract_text(JD_PATH)
    skills = extract_skill_keywords(jd_text)
    resume_text = extract_text(STRONG_MATCH_RESUME)
...
def test_guess_candidate_name_returns_first_line():
    """For a normal resume, the name heuristic should pick up the first line."""
...
def test_guess_candidate_name_falls_back_when_first_line_is_odd():
    """If the first line doesn't look like a name, fall back gracefully."""
